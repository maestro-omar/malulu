...
class RecurrentEvent extends Model
{
    ...
    public function calculateDate(\DateTime $from, \DateTime $to)
    {
        ...
        if ($this->easter_offset !== null) {
            ...
            $currentYearDate = $this->calculateEasterBasedDate($today->year, $this->easter_offset);
            if ($currentYearDate && $currentYearDate->between($fromCarbon, $toCarbon)) {
                return $currentYearDate;
            }

            // Check next year
            $nextYearDate = $this->calculateEasterBasedDate($today->year + 1, $this->easter_offset);
            if ($nextYearDate && $nextYearDate->between($fromCarbon, $toCarbon)) {
                return $nextYearDate;
            }

            // Check previous year
            $prevYearDate = $this->calculateEasterBasedDate($today->year - 1, $this->easter_offset);
    ...
    public function getOccurrencesInRange($from, $to): Collection
    {
        ...
        if ($this->easter_offset !== null) {
            for ($year = $startYear; $year <= $endYear; $year++) {
                $occurrenceDate = $this->calculateEasterBasedDate($year, $this->easter_offset);
    ...
    /**
     * Calculate a date based on Easter offset for a year, adjusting for weekday if specified.
     * Uses DateHelper for Easter calculation. Ensures Carnival dates fall on the correct weekday (Monday/Tuesday).
     *
     * @param int $year The year for which to compute the date
     ...
     */
    private function calculateEasterBasedDate(int $year, int $offset): ?Carbon
    {
        $baseDate = DateHelper::easterOffsetDate($year, $offset);
