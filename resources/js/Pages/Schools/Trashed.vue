<template>
  ...
  <AuthenticatedLayout>
    <template #admin-header>
      <AdminHeader  :title="`Escuelas Eliminadas`">
          ...
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div>
              ...
              <div class="hidden md:block">
                <table class="min-w-full divide-y divide-gray-200">
                  <thead class="bg-red-50">
                    <tr>
                      <th class="px-6 py-3 text-left text-xs font-medium text-red-600 uppercase tracking-wider">Nombre
                      </th>
                      <th class="px-6 py-3 text-left text-xs font-medium text-red-600 uppercase tracking-wider">CUE</th>
                      <th class="px-6 py-3 text-left text-xs font-medium text-red-600 uppercase tracking-wider">Localidad
                      </th>
                      <th class="px-6 py-3 text-left text-xs font-medium text-red-600 uppercase tracking-wider">Niveles
                      </th>
                      <th class="px-6 py-3 text-left text-xs font-medium text-red-600 uppercase tracking-wider">Eliminado
                      </th>
                      <th class="px-6 py-3 text-left text-xs font-medium text-red-600 uppercase tracking-wider">Acciones
                      </th>
                    </tr>
                  </thead>
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="(school, index) in schools.data" :key="school.id" :class="{
                      'bg-red-50': index % 2 === 0,
                      'bg-white': index % 2 === 1
                    }">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button v-if="hasPermission($page.props, 'school.delete')" @click="restoreSchool(school.id)"
                          class="text-green-600 hover:text-green-900 mr-3">
                          Restaurar
                        </button>
                        <button v-if="hasPermission($page.props, 'school.delete')" @click="forceDeleteSchool(school.id)"
              ...
              <div class="md:hidden space-y-4">
                <div v-for="(school, index) in schools.data" :key="school.id" :class="{
                  'bg-red-50': index % 2 === 0,
                  'bg-white': index % 2 === 1
