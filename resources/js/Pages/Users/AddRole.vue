                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <form @submit.prevent="submit" class="space-y-6">
                            <div v-if="form.errors.error" class="mb-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded">
                                {{ form.errors.error }}
                            </div>
                            <div class="space-y-6">
                                <!-- User Info -->
                                ...
                                <div v-if="selectedSchool" class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
                                        ...
                                        <button
                                            ...
                                            :class="[getLevelColorClasses({ code: 'unspecified' }), getLevelSelectedClass(null)]"
                                        >
                                            Sin especificar
                                        </button>
                                        ...
                                        <button
                                            ...
                                            :class="[getLevelColorClasses(level), getLevelSelectedClass(level)]"
                                        >
                                            {{ level.name }}
                                        </button>
                                    </div>
                                    <InputError class="mt-2" :message="form.errors.school_level_id" />
                                </div>
...
console.log('Schools with levels:', props.allSchools.map(school => ({
    id: school.id,
    name: school.name,
...
const filteredAvailableRoles = computed(() => {
    ...
    if (selectedSchool.value) {
        ...
        const hasSecondary = schoolLevels.some(level => level.code === 'secondary');
        ...
        if (selectedLevel.value) {
            ...
            if (selectedLevelCode === 'kinder') {
                roles = roles.filter(role => !['professor', 'class_assistant'].includes(role.code));
            } else if (selectedLevelCode === 'primary') {
                roles = roles.filter(role => !['professor', 'class_assistant'].includes(role.code));
            } else if (selectedLevelCode === 'secondary') {
                roles = roles.filter(role => !['grade_teacher', 'assistant_teacher', 'curricular_teacher'].includes(role.code));
            }
        }
...
watch(selectedSchool, (newSchool) => {
    ...
    if (newSchool?.id && !newSchool.school_levels) {
        ...
        axios.get(route('api.schools.show', newSchool.id))
            .then(response => {
...
const getLevelColorClasses = (level) => {
    const baseClasses = 'px-4 py-2 rounded-lg cursor-pointer transition-all duration-200 text-center';
    const colorClass = levelColors[level.code] || 'bg-gray-100 text-gray-800 border-gray-300';
    return `${baseClasses} ${colorClass}`;
};
    ...
    form.put(route('users.roles.store', props.user.id), formData, {
        ...
        onError: (errors) => {
            ...
            if (errors.error) {
                ...
                form.errors.error = 'Error al asignar el nuevo rol. Por favor, intente nuevamente.';
            }
        }
    });
};
