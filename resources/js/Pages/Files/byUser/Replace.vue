  ...
  <AuthenticatedLayout pageClass="q-pa-md row justify-center">
    <template #admin-header>
      <AdminHeader :title="`Reemplazar Archivo ${file.nice_name}`" :cancel="{
        href: route('users.file.show', { 'user': user.id, 'file': file.id }),
        label: 'Cancelar'
      }">
      </AdminHeader>
    </template>
              ...
              <form @submit.prevent="onSubmit">
                ...
                <div class="admin-form__actions">
                  <ActionButtons 
                    ...
                    :cancel-href="route('users.file.show', { 'user': user.id, 'file': file.id })"
                  />
                </div>
              </form>
            </div>
...
const onSubmit = () => {
  if (form.inputType === 'file' && form.file) {
    ...
    form.post(route('users.file.replace', { 'user': props.user.id, 'file': props.file.id }), {
      forceFormData: true
    })
