  ...
  <AuthenticatedLayout pageClass="q-pa-md row justify-center">
    <template #admin-header>
      <AdminHeader :title="`Reemplazar Archivo ${file.nice_name}`" :cancel="{
        href: route('school.file.show', { 'school': school.slug, 'file': file.id }),
        label: 'Cancelar'
      }">
      </AdminHeader>
    </template>
              ...
              <form @submit.prevent="onSubmit">
                ...
                <div class="admin-form__actions">
                  <ActionButtons 
                    ...
                    :cancel-href="route('school.file.show', { 'school': school.slug, 'file': file.id })"
                  />
                </div>
              </form>
            </div>
...
const onSubmit = () => {
  if (form.inputType === 'file' && form.file) {
    ...
    form.post(route('school.file.replace', { 'school': props.school.slug, 'file': props.file.id }), {
      forceFormData: true
    })
